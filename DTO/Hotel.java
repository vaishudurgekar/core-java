...
class Hotel{
   ...
   public boolean updateFoodPriceByName(String name, int price){
   ...
   public boolean deleteFoodsByName(String name){
	...
	public boolean deletFoodByType(String type){
		boolean foodsDeleted = false;
		System.out.println("Inside deletFoodItemByType()");
		int i,j;
		for(i=0, j=0; j<foodItems.length; j++){
			if(!foodItems[j].getType().equals(type)){
				foodItems[i++] = foodItems[j];
				foodsDeleted = true;
				System.out.price("Enter the type to be deleted");
			}
			else{
				System.out.println("Foods type is not found");
			}
		}
		foodItems = Array.copyOf(foodItems, i);
		return foodsDeleted;
	}
}